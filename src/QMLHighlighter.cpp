...
{
    if (!m_cacheLoaded) {
        loadDictionary(":/syntax/keywords.txt", m_keywordsCache);
        loadDictionary(":/syntax/javascript.txt", m_jsIdsCache);
        loadDictionary(":/syntax/qml.txt", m_qmlIdsCache);
        loadDictionary(":/syntax/properties.txt", m_propertiesCache);
        m_cacheLoaded = true;
    }
}
